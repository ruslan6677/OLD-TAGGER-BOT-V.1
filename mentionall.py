...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("__Bu əmir yalnız qruplar üçün etibarlıdır!__")

  admins = []
  async for admin in client.iter_participants(event.chat_id, filter=ChannelParticipantsAdmins):
    admins.append(admin.id)
  if not event.sender_id in admins:
    return await event.respond("__Bu əmirdən yalnız idarəçilər isdifadə edə bilər!__")
  ...
  if event.pattern_match.group(1):
    ...
    msg = event.reply_to_msg_id
    if msg == None:
        return await event.respond("__Mesajlar üşün kullanıclı tag edəmərəm!(Qrupa əlavə etmədən öncə göndərilən mesajlar)__")
  elif event.pattern_match.group(1) and event.reply_to_msg_id:
    return await event.respond("__Başlamaq üçün bir səbəb yoxdur!__")
  else:
    return await event.respond("__Taga başlamaq üçün səbəb yazın!__")

  if mode == "text_on_cmd":
    anlik_calisan.append(event.chay_id)
    usrnum = 0
    usrtxt = ""
    async for usr in client.iter_participants(event.chat_id):
      usrnum += 1
